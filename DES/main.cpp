...
void read_and_write_in_file(char inputFolderPath[], char outputFolderPath[], char keyFolderPath[], enum operation op);
...
*/
/*arguments input is
	argv[1] : input folder path
	argv[2] : output folder path
	argv[3] : key folder path
	arv[4]  : operation
...
int main(int argc, char* argv[])
{
	...
	{
		read_and_write_in_file(argv[1], argv[2], argv[3], ENCRYPT);
		std::cout << "encryption running...." << std::endl;
	}
	else if (!strcmp(argv[4], "decrypt"))
	{
		read_and_write_in_file(argv[1], argv[2], argv[3], DECRYPT);
...
void read_and_write_in_file(char inputFolderPath[], char outputFolderPath[], char keyFolderPath[], enum operation op)
{
	...
	*/
	...
	ptr = fopen(inputFolderPath, "r");
	ptr3 = fopen(keyFolderPath, "r");
	...
	do
	{
		...
		if (counter == 16)
		{ // 4*16=64 so the 64 bit of 'data' is consumed
